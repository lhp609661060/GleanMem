...
> **评审对象**：`01-design.md` v3.1 方案（权威设计）+ 现有 v2 代码实现（`gleanmem/backend`，1356 行，无 git 元数据）
...
- **总体判断**：方案方向成立、Spike 已验证核心通路（Dify MCP SSE + `X-Agent-ID` 透传 + Agent 主动调工具），v3.1 的收件箱/身份/观察 push/codebase 蒸馏建模**在纸面上自洽**；但**现有代码仍是 v2 形态，01-design §实现现状与差距清单低估了 v3 语义的实现量，且漏了 2 个连开发环境都起不来的 🔴（P1-1 数据丢失未修复、`uv run gleanmem` 启动入口缺失）**。
- **可开工性**：**可以按 V1 Week 1 开工，但开工第一步必须先修"建表链路 + 启动入口"两件事**，否则 `docker compose up` + `alembic upgrade head` + `uv run gleanmem` 三个文档化命令里有 2 个直接失败。
- **TOP 3 开工前必处理**（详见 §8）：
...
- **位置**：`core/learning.py:90-115`（`_run` 无 decisions 判空直接执行 `:106-110` 的全量 `delete`）；对照 `docs/gleanmem/05-solutions.md:190-229`（P1-1"LLM 分析失败时事件不丢失"）
...
### 🔴 N2：`uv run gleanmem` 启动入口缺失（文档化命令即失败）

- **位置**：`backend/pyproject.toml:25`：`gleanmem = "gleanmem.main:main"`；`src/gleanmem/main.py`（50 行）**没有 `def main()`**（grep `def main` 全包零命中）；`AGENTS.md:21` 文档化 `uv run gleanmem` 为启动命令
- **问题**：console script 指向不存在的入口，`uv run gleanmem` 直接 `AttributeError`。
- **影响**：按仓库文档第一步启动服务即失败；演示/评审第一印象差。
- **修复建议**：二选一——在 `main.py` 加 `def main(): uvicorn.run("gleanmem.main:app", host=settings.host, port=settings.port)`；或把 entry point 改为 `gleanmem.main:app` 并删掉 console script。顺带修正 `AGENTS.md` 命令描述。
- **验证方式**：`uv run gleanmem` 起服务 → `/health` 返回 `{"status":"ok"}`。
...
- **说明**：与 gleanmem 服务表（`agent_spaces` 等）同库并存、未隔离 schema——演示时注意命名冲突/混库；配合观察 push feeder 使用是合理的演示资产，属 Week 2 产物。
...
**可以按 V1 Week 1 开工。** 方案方向与 Spike 结论支持继续；差距清单 10 条全部属实可执行；没有发现需要推翻方向的架构级问题。**但开工的第一天不能做"补 pytest 或观察入站"，必须先把两个让开发环境起不来的 🔴 修掉**（建表链路 + 启动入口），否则 `alembic upgrade head` 与 `uv run gleanmem` 两个文档化命令当场失败。
...
3. **一次性落地 v3 契约迁移 + 删除 query_db + 修启动入口**（🔴/🟡）：`source`/`dedup_key`/`api_key_hash`/`api_key_prefix`/`learning_logs.source` 加列 + dedup 部分唯一索引；flush 契约改为从 key 解析身份（Week 2 身份层的前置）；`mcp/tools.py` 回到 3 个工具；`main.py` 补 `def main()`（或改 entry point）。验证：`uv run gleanmem` 起服务；MCP tools 列表 3 个；`uv run pytest` 绿。
...
| 主题 | 证据 |
|------|------|
| 迁移默认值损坏 | `gleanmem/backend/alembic/versions/edac4c069c8c_initial_all_5_tables.py:25` |
| 触发器函数外置 | `edac4c069c8c_initial_all_5_tables.py:100-102`；`backend/init-db.sql:5-19` |
| 'simple' 触发器 | `backend/init-db.sql:7,15` |
| compose 无 zhparser | `gleanmem/docker-compose.yml:3` |
| zhparser Dockerfile 断裂 | `gleanmem/docker/Dockerfile.pg-zhparser:1,14`；`docker/` 目录无 init-db.sql |
| load_memory 越权 | `backend/src/gleanmem/mcp/server.py:51-54` |
...
| main 入口缺失 | `backend/pyproject.toml:25`；`src/gleanmem/main.py`（无 `def main`）；`AGENTS.md:21` |
...
| 模板仍传 agent_id | `gleanmem/docs/dify-workflow-template.yml`（头部注释 + Flush 节点） |
