...
- `yd-memory-service/backend/` holds ~1356 lines of FastAPI/Python implementing the v2 direction; it needs the fix list from `01-design.md` §实现现状与差距清单 applied before it matches v3.
- `spike/` holds disposable Dify/MCP verification scripts + `SPIKE-REPORT.md` (Spike passed).
- Not a git repo. Commands (uv, docker compose, alembic, pytest) are documented in `AGENTS.md`.
