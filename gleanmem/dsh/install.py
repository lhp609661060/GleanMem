...
4. 把 MCP 插件实例合并进 DSH profile 的 cordis.patch.yml（serverName=gleanmem）
...
def start_backend(backend: Path, base_url: str) -> None:
    ...
    with open(log_file, "ab") as f:
        subprocess.Popen(
            [uv, "run", "uvicorn", "gleanmem.main:app", "--host", "127.0.0.1", "--port", port],
            cwd=str(backend),
            env=env,
            stdout=f,
...
def mcp_entry_text(backend: Path, agent_id: str) -> str:
    ...
    stdio_server = backend / "src" / "gleanmem" / "mcp" / "stdio_server.py"
...
def merge_patch_file(target: Path, entry_text: str) -> None:
    ...
    log(f"    已写入 MCP 插件实例（id={MCP_ENTRY_ID}, serverName=gleanmem）")
    if backup:
        log(f"    原文件备份：{backup}")
...
def main() -> None:
    ...
    log(f"  2. MCP 工具：重启 DSH（或等待 HMR 热载）后，Agent 会看到 mcp__gleanmem__recall / mcp__gleanmem__load_memory / mcp__gleanmem__memorize")
