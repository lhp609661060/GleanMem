"""gleanmem-distill：codebase 蒸馏客户端 CLI（V1.5a）。
...
用法：
    ...
    uv run gleanmem-distill scan <repo>

    # 2. 全量蒸馏 + 上传知识卡 + 落盘 md 投影
    uv run gleanmem-distill run <repo> --url http://localhost:8000 --key <space_key>

    # 3. 只重建 md 投影（卡是事实源，md 可随时重建）
    uv run gleanmem-distill sync <repo> --url ... --key ... [--all]

    # 4. 增量：把本地相对 --since 的变更推进收件箱（CI 提交后回调用）
    uv run gleanmem-distill refresh <repo> --url ... --key ... --since HEAD~1
"""
...
from gleanmem.core.codebase.projection import render_markdown, write_projection
...
def cmd_scan(args: argparse.Namespace) -> int:
    ...
    print()
    print("这是 dry-run，未调用 LLM、未产生费用。确认后运行 `gleanmem-distill run` 执行蒸馏。")
    return 0
...
async def _run(args: argparse.Namespace) -> int:
    ...
    async with httpx.AsyncClient(base_url=args.url, headers=headers, timeout=120) as http:
        ...
        for d in drafts:
            ...
            written += ok
            protected += not ok
        print(f"md 投影：写入 {written} 个文件，本地 protected 跳过 {protected} 个 → {args.repo}/.gleanmem/wiki/")
        return 0
...
async def _refresh(args: argparse.Namespace) -> int:
    ...
    from gleanmem.core.codebase.scanner import (
    ...
    async with httpx.AsyncClient(base_url=args.url, headers=headers, timeout=60) as http:
        ...
        print("变更已进收件箱，下次 flush 时按模块重生成知识卡；之后运行 `gleanmem-distill sync` 更新 md。")
        return 0


def main() -> None:
    parser = argparse.ArgumentParser(prog="gleanmem-distill", description="codebase 蒸馏客户端")
