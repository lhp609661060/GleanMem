...
<template>
  ...
  <div v-if="tab === 'dify'" class="docs">
    <div class="card">
      ...
      <h3>前置条件</h3>
      <ul>
        <li>yd-memory-service 已启动（本机默认 <span class="mono">http://127.0.0.1:8000</span>）。</li>
  ...
  <div v-else class="docs">
    <div class="card">
      ...
      <p class="muted">
        yd-memory-service 是<b>独立部署的服务</b>，不是 DSH 插件——DSH 侧无需「安装」，只需通过
...
curl -X POST http://127.0.0.1:8000/api/v1/learning/flush \
  -H "Authorization: Bearer $SPACE_KEY"</pre>
      ...
      <pre>cd yd-memory-service/backend
