...
description: >
  ...
  service yd-memory-service: recall memories before answering, memorize user
  feedback or corrections, flush learning at session end, and inspect audit
  logs.
---

# yd-memory-service REST 客户端（DSH 接入）

让 DSH Agent 通过 REST 使用外部记忆平台 yd-memory-service。身份 = per-space API Key。

## 前置

- 服务地址：`http://127.0.0.1:8000`（后端 `yd-memory-service/backend`）
- 后端启动命令：
  ```bash
  cd <repo>/yd-memory-service/backend
  PYTHONPATH=src uv run uvicorn yd_memory_service.main:app --host 127.0.0.1 --port 8000
  ```
