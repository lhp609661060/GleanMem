"""md 投影渲染（D11）：知识卡 → <repo>/.yd-memory/wiki/*.md。

方向单一：卡 → md，永不反向。md 可重建 ⇒ 不参与审计不变式。
protected 保护：本地 md 首行含 `<!-- yd-memory: protected -->` 的文件不被覆盖。
"""
...
PROTECTED_MARKER = "<!-- yd-memory: protected -->"
WIKI_SUBDIR = ".yd-memory/wiki"
...
def render_markdown(
    *,
    ...
    lines = [
        ...
        "",
        "> 本文件由 yd-memory-service 从知识卡渲染生成（单向投影）。",
