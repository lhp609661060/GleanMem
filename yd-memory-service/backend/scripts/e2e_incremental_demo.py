...
from yd_memory_service.core.codebase.scanner import fingerprint, scan_repo
from yd_memory_service.core.codebase.store import card_id
...
async def main():
    ...
    async with httpx.AsyncClient(transport=t, base_url="http://test", headers=H) as c:
        ...
        (repo / ".yd-memory" / "wiki" / "api.md").write_text(f"{PROTECTED_MARKER}\n# 我手写的 API 说明\n")
        ...
        api_md = (repo / ".yd-memory" / "wiki" / "api.md").read_text()
